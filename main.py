...
def Task2():
    clear_window(root)
    root.title("Рух тіла підкинутого під кутом з опором повітря")
    ...
    def start_simulation():
        ...
        def update_motion():
            ...
            # Оновлення швидкості з опором повітря
            #Vx = Vx * math.e ** (-k/m * dt)
            Vy = (-Vy + m * g / k) * math.e ** (-k/m * dt) - m * g / k
            Vx = Vx * math.e ** (-k/m * dt)
            #Vy = m/k * (v0*math.sin(theta) + m*g/k)*(1 - math.e ** (-k/m * dt)) - m*g*dt/k
            ...
            if y < GROUND_Y - BALL_SIZE:
                ...
                canvas.create_line(x + BALL_SIZE, GROUND_Y, x + BALL_SIZE, GROUND_Y - 50, fill="black", width=2)
                canvas.create_text(x, GROUND_Y + 10, text=f"Відстань: {(x-50)/SCALE:.2f} м", fill="black", font=('Arial', 10))
