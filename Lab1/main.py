...
input_frame.pack(side="left", fill="y", expand=True)
...
tk.Label(input_frame, text="Початкова швидкість (м/с):").grid(row=3, column=0, padx=10, pady=10)
entry_v0 = tk.Entry(input_frame)
entry_v0.grid(row=3, column=1, padx=10, pady=10)

tk.Label(input_frame, text="Прискорення (м/с²):").grid(row=4, column=0, padx=10, pady=10)
...
tk.Button(input_frame, text="Очистити", command=clean_window).grid(row=5, column=0, columnspan=2, pady=10)
tk.Button(input_frame, text="Запустити симуляцію", command=start_simulation).grid(row=6, column=0, columnspan=2, pady=10)
tk.Button(input_frame, text="Отримати графік", command=plot_trajectory).grid(row=7, column=0, columnspan=2, pady=10)
